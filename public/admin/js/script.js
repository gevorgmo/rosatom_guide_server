...
$(document).ready(function(){
	...
	$("body").on('click', '#media_set', function(event) {
		if($('.media_itm.selected')[0]){
			...
			var _filname=$('.media_itm.selected').attr('data-file');
			_itm.attr('data-src', _src);
			_itm.attr('data-file', _filname);
			_itm.find('div').text(_filname);
	...
	$('body').on('click', '.btn-saveitem', function(evt) {
		...
		if($('.prtitle')[0]){
			$('.prtitle').each(function(){
				...
				if($(this).attr('data-lang')=="ru" && $(this).val().trim()=="" && ($('#prcategory').val()=="event" ||  $('#prcategory').val()=="exhibition")){
					noerror=false;
				}
			});
		}
		...
		if($('.prtranscript')[0]){
			$('.prtranscript').each(function(){
				...
				if($(this).attr('data-lang')=="ru" && ($('#prcategory').val()=="event" || ($('#prcategory').val()=="media" && _data["media_type"]!="4" &&  _data["media_type"]!="2" && _data["media_type"]!="3" && _data["media_type"]!="1")) && $(this).val().trim()==""){
					noerror=false;
				}
			});
		}
		...
		if($('.praudio')[0]){
			$('.praudio').each(function(){
				if($(this).attr('data-src')!="") {
					_data['audio'][$(this).attr('data-lang')]=$(this).attr('data-src');
					_data['audio'][$(this).attr('data-lang')+"file"]=$(this).attr('data-file');
				}
			});
		}
		...
		if($('.prtxt')[0]){
			$('.prtxt').each(function(){
				...
				if($(this).attr('data-lang')=="ru" && $(this).attr('data-id')=="type" && $(this).val().trim()==""){
					noerror=false;
				}
			});
		}	
...
////////////////////////////////////////////////
var _validFileExtensions = [".jpg",".jpeg",".png",".svg",".gif",".mp4",".ogv",".mp3",".aac",".m4a"];    
