...
$(document).ready(function() {
	...
	$('body').on('click', '.inner_logo_block', function(evt) {
		evt.preventDefault();	
		$('.guide_blocks_container').hide();	
		$('#global_wrap').show();
		if(document.body.className=="scanning") {
	...
	$('body').on('click', '.lang_cont', function(evt) {
		...
		if(!$(this).hasClass('selected')){
			LanguageLoad($(this).attr('data-lang'));
		}
		return false;
	});	
	...
	$('body').on('click', '.media_player_speed_list_it', function(evt) {
		evt.preventDefault();	
		_playbackRate=(parseFloat($(this).attr('data-val'))==1 ? 1.0 :1.25); 
		if(_media_type=="1"){
			_audio.playbackRate=_playbackRate;
		}
		$('.media_player_speed_list_it').removeClass('selected');
		$(this).addClass('selected');
		return false;
	});	
...
function GetContent(_url, _cb){
	...
	$.get(_url+"?uuid="+(typeof _uuid!="undefined" ? _uuid : "test"), function(data, status){
		...
		if(_url.indexOf('/maps')>-1){
			...
			if(!$('body').hasClass('blackbg')) $('body').addClass('blackbg');	
		}
		...
		if($('#media_content_data')[0]){
			...
			if(_media_type=="1"){
				$('.media_player_speed_list_it'+(_playbackRate==1 ?1 : 2)).addClass('selected');
			}
			...
			if(_media_type!="4" && _media_type!="3"){
				...
				///////////////////////// OGNENSK
				if (typeof SendUDP === "function" && _media_id=="c6")  SendUDP("run00", "10.0.121.14",  6024, function(error){if(error) console.log(error);});

				
			} else {
				console.log("start;"+_media_id);
				$('.loader_start').css({'visibility':'hidden','opacity':'0'});
				if (typeof SendUDP === "function")  SendUDP("start;"+_media_id, "10.0.121.2",  6025, function(error){ if(error) console.log(error); });
...
function playAudio() {
	...
	if(_playstatus){
		if(_media_type=="1"){
			_playButton.className="player_button playing";
			_audio.playbackRate=_playbackRate;
		} else {
			_audio.playbackRate=1;
		}
	...
	if(ev.touches){
		if(ev.touches.length>0){
			...
			if (ev.touches.length === 2) {
				_zooming=true;
				prevDiff=Math.max(Math.abs(ev.touches[0].pageX - ev.touches[1].pageX), Math.abs(ev.touches[0].pageY - ev.touches[1].pageY));
			} else {
				_zooming=false;
			}
		}
...
function pointermove_handler(ev) {
	...
	if(ev.touches){
		if (ev.touches.length === 2) {
			var curDiff = Math.max(Math.abs(ev.touches[0].pageX - ev.touches[1].pageX), Math.abs(ev.touches[0].pageY - ev.touches[1].pageY));
			sclsize+=(curDiff-prevDiff)/400;		
			prevDiff=curDiff;
			prevX = ev.touches[0].clientX;
			prevY = ev.touches[0].clientY;	
			gotozoom();
		} else if(!_zooming){
			curpX+=ev.touches[0].clientX-prevX;
			curpY+=ev.touches[0].clientY-prevY;	
			if(curpX>10) curpX=10;	
			if(curpX<(-(sclsize-1)*390+10))curpX=(-(sclsize-1)*390+10);
			if(curpY>10) curpY=10;
			if(curpY<-((sclsize-1.3)*560+10))	curpY=-((sclsize-1.3)*560+10);
			prevX = ev.touches[0].clientX;
			prevY = ev.touches[0].clientY;
			if(sclsize<1.3){
				curpY=80-(sclsize-1)*350;
			}
			if(sclsize==1){
				curpX=10;
				curpY=80;
			}			
			document.getElementById("main_map_img").style.backgroundPosition=curpX+"px "+curpY+"px";
		}
	}
}
///////////////////////////////////////////////////////
function zoom_init() {
	var el=document.getElementById("main_map_img");
	el.addEventListener("touchstart", pointerdown_handler, false);
	el.addEventListener("touchennd", pointerup_handler, false);
	el.addEventListener("touchmove", pointermove_handler);
}
////////////////////////////////////////////////////////
function gotozoom(){
	if(sclsize>2.2) sclsize=2.2;
	if(sclsize<1) sclsize=1;
	document.getElementById("main_map_img").style.backgroundSize=(sclsize*390)+"px";
